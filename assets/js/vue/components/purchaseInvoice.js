const purchaseInvoice = Vue.component('purchase-invoice', {
                        ...
                        <table _a584de>
                            ...
                            <tbody>
                                <tr v-for="(product, sl) in cart">
                                    ...
                                    <br> (
                                        <span v-for="(IMEICart, ind) in imeiStore.filter(prod_sl => prod_sl.ps_prod_id == product.Product_IDNo) ">
                                            {{IMEICart.ps_imei_number}}<span v-if="(imeiStore.length  > -1) > ind">, </span>
                                        </span>
                                    )
                                    </span>
                                    </td>
    ...
    methods: {
        ...
        async getPurchaseImei() {
            await axios.post('/get_purchases_imei', { purchaseId: this.purchase_id }).then(res => {
