				...
				<div class="modal-wrapper">
					<div class="modal-container">
						...
						<div class="modal-header">
							<slot name="header">
								<h3>IMEI Number List</h3>
							</slot>
						</div>
						<div class="modal-body" style="overflow: hidden;height: 100%;margin: -8px -14px -44px;">
							<slot name="body">
								<div class="form-group">
									<div class="col-sm-12" style="display: flex;margin-bottom: 10px;"></div>
								</div>
							</slot>
							<table class="table">
								<thead>
				...
				<table class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td>{{ product.is_imei != 'false' ? product.purchase_total_am : product.stock_value }}</td>
							<td><button v-if="product.is_imei != 'false'" type="button" class="button edit" @click="viewIMEI(product.product_id)">
									<i class="fa fa-eye"></i>
								</button></td>
						</tr>
					</tbody>
				...
				<table class="table table-bordered" v-if="searchType != 'current' && searchType != null" style="display:none;" v-bind:style="{display: searchType != 'current' && searchType != null ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td>{{ product.current_quantity }} {{ product.Unit_Name }}</td>
							<td>{{ product.is_imei != 'false' ? product.purchase_total_am : product.stock_value }}</td>
							<td><button v-if="product.is_imei != 'false'" type="button" class="button edit" @click="viewIMEI(product.Product_SlNo)">
									<i class="fa fa-eye"></i>
								</button></td>
						</tr>
					</tbody>
		...
		methods: {
			...
			getStock() {
				...
				axios.post(url, parameters).then(res => {
					if (this.searchType == 'current') {
						this.stock = res.data.stock.filter(fil => fil.current_quantity > 0);
					}else{
					...
					console.log(this.totalStockValue);
				})
			},
