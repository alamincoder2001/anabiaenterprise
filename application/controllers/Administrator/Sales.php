...
class Sales extends CI_Controller
{
    ...
    public function imeiHistory()
    {
        ...
        $data['title'] = "IMEI History";
        $data['content'] = $this->load->view('Administrator/imei_history', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function imeiList()
    {
        $imeiList =   $this->db->query("select ps_imei_number from tbl_product_serial_numbers where ps_brunch_id=?", [$this->session->userdata('BRANCHid')])->result();
        echo json_encode($imeiList);
    }

    public function get_imei_report()
    {
        $data = json_decode($this->input->raw_input_stream);
        $res['saleReport'] =  $this->db->query("select c.*,sm.SaleMaster_InvoiceNo as invoiceNo,sm.SaleMaster_SaleDate as saleDate,sn.ps_imei_number,
        ...
        where  sn.ps_imei_number=? ", [$data->imei])->result();

        $res['purchaseReport'] =  $this->db->query("select s.*,pm.PurchaseMaster_InvoiceNo as invoiceNo,pm.PurchaseMaster_OrderDate as purchaseDate,sn.ps_imei_number,
    ...
    public function returnIemi()
    {
        ...
        $data['content'] = $this->load->view('Administrator/sales/imei_return', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    function get_return_imei_number()
    {
        ...
        $imei_detail = $this->db->query("select ps.*,pd.SaleDetails_Rate,p.Product_Name,p.Product_SlNo
        ...
        foreach ($imei_detail as $key => $value) {
            ...
            $html .= '</tr>
                     ...
                     <td style="text-align: center;">' . $value->SaleDetails_Rate . '</td>
                     <td style="display: flex;align-content: center;justify-content: center;"><button style="width: 118px;padding: 7px;margin: 2px;border: navajowhite;" class="btn btn-success btn-sm" id="parchase_return_btn" data-id="' . $value->ps_imei_number . '" data-prod_id="' . $value->Product_SlNo . '" data-amount="' . $value->SaleDetails_Rate . '"> Return</button></td>
                 </tr>';
        }
        $html .= '</table>';
        echo $html;
    ...
    public function sale_imei_return()
    {
        $data = json_decode($this->input->raw_input_stream);
    ...
    function GetIMEIList()
    {
        ...
        $imei = $this->db->query("SELECT ps.*,ps.ps_s_discount as SaleDetails_Discount,ps.ps_prod_id,p.*,u.Unit_Name,pc.ProductCategory_Name, concat(p.Product_Name, ' - ', p.Product_Code) as display_text
        FROM  tbl_product_serial_numbers as ps
        INNER JOIN tbl_product as p 
    ...
    function  GetIMEIListService()
    {
        $branch_id = $this->session->userdata("BRANCHid");
        $imei = $this->db->query("SELECT ps.ps_imei_number,ps.purchase_date,ps.ps_prod_id,p.*,u.Unit_Name,pc.ProductCategory_Name
        FROM  tbl_product_serial_numbers as ps
        INNER JOIN tbl_product as p 
        ON ps.ps_prod_id = p.Product_SlNo
        INNER JOIN tbl_unit as u 
    ...
    {
        $data = json_decode($this->input->raw_input_stream);
        $IMEI = $this->db->query("SELECT * FROM tbl_services_details WHERE s_d_imei=?", $data->IMEI)->num_rows();
        echo json_encode($IMEI);
    }
    public function addSales()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                if ($cartProduct->is_imei != 'false') {
                    foreach ($cartProduct->IMEIStore as $key => $value) {
                        $data = array(
                            'ps_s_status' => 'yes',
                            'ps_s_r_status' => 'no',
                            'ps_s_r_amount' => 0,
                        ...
                        $this->db->where('ps_imei_number', $value->ps_imei_number);
                        $this->db->update('tbl_product_serial_numbers', $data);
                    }
                }
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                foreach ($cartProduct->IMEIStore as $keyimei => $valueimei) {
                    // Insert IMEI
                    // $check_available = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE ps_imei_number=?",$valueimei->ps_imei_number)->num_rows();
                    ...
                    // $this->db->where('sales_details_id', $product->SaleDetails_SlNo);
                    $this->db->where('ps_imei_number', $valueimei->ps_imei_number);
                    $this->db->update('tbl_product_serial_numbers', $dataArr);
    ...
    public function remove_cart_imei()
    {
        $data = json_decode($this->input->raw_input_stream);
        foreach ($data as $key => $value) {
            $data = array(
